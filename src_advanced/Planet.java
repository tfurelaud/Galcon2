...
public class Planet implements Serializable{
	...
	/** The number of vessels and strong vessels on the planet.
	 *  The number of a vessel. */
	private int nbVessels, nbStrongVessels, numvess = 0;
	...
	/** The select strong. */
	private boolean selectStrong = false;
	 ...
	 * @param nbStrongVes the number of strong vessels.
	 * @param prop the first proprietor.
	 * @param pressed the pressed
	 * @param g the current game.
	 */
	Planet( Circle planet, int ProdTime, int nbVessels, int nbStrongVes, Player prop,boolean pressed, Game g){
		this.x = planet.getCenterX();
		this.y = planet.getCenterY();
		this.r = planet.getRadius();
		this.ProdTime = ProdTime;
		this.nbVessels = nbVessels;
		this.nbStrongVessels = nbStrongVes;
		this.prop = prop;
		this.game = g;
	}
	...
	/**
	 * Gets the number of strong vessels on the planet.
	 *
	 * @return the the number of strong vessels on the planet.
	 */
	public int getNbStrongVessels() {
		return nbStrongVessels;
	}


	/**
	 * Sets the number of strong vessels.
	 *
	 * @param nbStrongVessels the new number of strong vessels on the planet.
	 */
	public void setNbStrongVessels(int nbStrongVessels) {
		this.nbStrongVessels = nbStrongVessels;
	}


	/**
	 * Checks if is select strong.
	 *
	 * @return true, if the planet is selected.
	 */
	public boolean isSelectStrong() {
		return selectStrong;
	}


	/**
	 * Useful to know if the user wants to know the strong vessel number on the planet.
	 *
	 * @param selectStrong the new value.
	 */
	public void setSelectStrong(boolean selectStrong) {
		this.selectStrong = selectStrong;
	}
	...
	/**
	 * Stroke the planet when the user want to know the number of strong vessels on it.
	 *
	 * @param gc the graphicsContext.
	 */
	public void stroke(GraphicsContext gc) {
		...
		gc.strokeOval(x-r,y-r,r*2+1,r*2+1);
		gc.fillText(Integer.toString(nbStrongVessels), x-24, y+6);
	}
